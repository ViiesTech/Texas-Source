...
const EditProfile = () => {
  ...
  const [state, setState] = useState({
    ...
    company_image: {
      name: user?.companyImage ? user?.companyImage : '',
      uri: ''
    },
    phone: user?.phone || '',
    user_image: '',
  ...
  const onSelectPhoto = async () => {
    let status = await requestPermission('media');
    if (status === 'granted') {
      const options = {
        title: 'Select Image',
        storageOptions: {
...
console.log('state.company_image',state.company_image.uri)
console.log('user?.companyImage',user?.companyImage)
  const onUpdateProfile = async () => {
    ...
    if (state.company_image) {
      data.append('companyImage', {
        ...
        uri:
          Platform.OS === 'android'
            ? state.company_image.uri
            : state.company_image.uri.replace('file://', ''),
      });
    }
    if (state.user_image) {
        ...
        <TouchableOpacity
          ...
          <Image
            source={
              user?.UserProfile
                ? {
                    uri: `https://appsdemo.pro/Texas_Server/${user?.UserProfile}`,
                  }
                : state?.user_image
                ? {uri: state.user_image}
                : Images.user
            }
        ...
        <View style={styles.inputContainer}>
          ...
          />
            <TouchableOpacity onPress={() => onSelectImage()}>
                <Input
                  value={state.company_image.name}
                  style={styles.inputStyle}
          ...
          <Button
            ...
            buttonStyle={{width: responsiveWidth(88)}}
          />
        </View>
      </ScrollView>
    </Container>
